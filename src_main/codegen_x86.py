...
def manager_x86(cppName, headerName, threadList, signatureSize, regBitWidth, numCores, strideType):
    cppString  = ""
    cppString += "/* Some code is adopted from paging.S & smp.S in x86 bare-metal programming examples */\n"
    cppString += "/* https://github.com/cirosantilli/x86-bare-metal-examples */\n"
    cppString += "#include \"%s\"\n" % headerName
    cppString += "#include \"common.h\"\n"
    cppString += "    .globl classify_result_binary\n"
    cppString += "BEGIN\n"
    cppString += "    CLEAR\n"
    cppString += "    STAGE2_LBA\n"
    cppString += "    ACTIVATE_A20_KBD\n"
    cppString += "    cli\n"
    cppString += "    PROTECTED_MODE\n"
    cppString += "    SETUP_PAGING_IA32E_1G\n"
    cppString += "    PAGING_IA32E_ON\n"
    cppString += "    ENTER_64BIT_MODE\n"
    cppString += "    IDT_IA32E_SETUP_48_ISRS\n"
    cppString += "    REMAP_PIC_32\n"
    cppString += "    PIT_GENERATE_FREQUENCY\n"
    cppString += "    PIT_SET_FREQ 50000  /* Real machine: Each tick is 20us. */\n"
    cppString += "    #PIT_SET_FREQ 5000  /* QEMU: Each tick is 200us. */\n"
    cppString += "    sti\n"
    # DEBUG
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0F800, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FA00, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FC00, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FE00, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FF00, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FFE0, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x0FFF0, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x10000, $16\n"  # FIXME: delete it
    #cppString += "    VGA_PRINT_BYTES_64BIT $0x10010, $16\n"  # FIXME: delete it
    #cppString += "    jmp .\n"
    # END DEBUG
    cppString += "    movb $0, thread_spawn_lock\n"
    cppString += "    movb $0, thread_join_lock\n"
    cppString += "    movb $0, TEMP_DATA0\n"
    cppString += "#ifdef EXEC_SYNC\n"
    cppString += "    movb $0, thread_exec_barrier0\n"
    cppString += "    movb $0, thread_exec_barrier1\n"
    cppString += "    movb $0, thread_exec_barrier_ptr\n"
    cppString += "#endif\n"
    cppString += "    /* Copy thread code */\n"
    cppString += "    cld\n"
    #for thread in threadList:
    #    cppString += "    mov $thread%d_length, %%ecx\n" % thread
    #    cppString += "    mov $thread%d_routine, %%esi\n" % thread
    #    cppString += "    mov $TEST_THREAD_BASE_%d, %%edi\n" % thread
    #    cppString += "    rep movsb\n"
    cppString += "    /* Create test data section */\n"
    cppString += "    mov $0x0, %ecx\n"
    cppString += "    mov $0xFFFF0000, %ebx\n"
    cppString += "    mov $TEST_DATA_SECTION, %edi\n"
    cppString += "init_data_section:\n"
    cppString += "    lea (%ebx,%ecx,1), %eax\n"
    cppString += "    mov %eax, (%edi)\n"
    if (strideType == 0):
        cppString += "    add $0x4, %edi  // strideType = 0\n"
    elif (strideType == 1):
        cppString += "    add $0x10, %edi  // strideType = 1\n"
    elif (strideType == 2):
        cppString += "    add $0x40, %edi  // strideType = 2\n"
    else:
        assert(False)
    cppString += "    inc %ecx\n"
    #cppString += "    cmp $TEST_DATA_LOCATIONS,%ecx\n"
    cppString += "    cmp $NUM_SHARED_DATA,%ecx\n"
    cppString += "    jl init_data_section\n"
    cppString += "    /* Initialize test BSS section */\n"
    cppString += "    mov $0x0, %edx\n"
    cppString += "    mov $TEST_BSS_SECTION, %edi\n"
    cppString += "init_bss_section:\n"
    cppString += "    mov $0x00000000, %edx\n"
    cppString += "    mov %edx, (%edi)\n"
    cppString += "    add $0x4, %edi\n"
    cppString += "    cmp $TEST_BSS_SECTION + TEST_BSS_SIZE_PER_THREAD * NUM_THREADS, %edi\n"
    cppString += "    jl init_bss_section\n"
    #cppString += "    VGA_PRINT_BYTES_64BIT $TEST_DATA_SECTION, $16\n"
    #cppString += "    VGA_PRINT_BYTES_64BIT $TEST_BSS_SECTION, $16\n"
    cppString += "    /* Load address of ICR low dword into ESI. */\n"
    cppString += "    mov PIC_ICR_LOW_ADDRESS, %esi\n"
    cppString += "    /* Broadcast INIT IPI to all APs */\n"
    cppString += "    mov $0x000C4500, %eax\n"
    cppString += "    /* Broadcast INIT IPI to all APs */\n"
    cppString += "    mov %eax, (%esi)\n"
    cppString += "    /* 10-millisecond delay loop. */\n"
    cppString += "    PIT_SLEEP_TICKS $500\n"
    cppString += "    movl $0x11111111, TEMP_DATA0\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $4\n"
    # Reversed-order core allocation (thread 0 -> core 3, thread 1 -> core 2, ... thread 3 -> core 0)
    cppString += "    # Assume core 0 is boot-strap processor\n"
    threadIdx = 0
    for thread in threadList:
        if thread == (numCores-1):
            continue
        cppString += "    # THREAD %d\n" % thread
        cppString += "    mov $0x%02d000000, %%eax\n" % ((numCores-1) - threadIdx)
        cppString += "    mov PIC_ICR_HIGH_ADDRESS, %esi\n"
        cppString += "    mov %eax, (%esi)\n"
        cppString += "    mov $thread%d_routine, %%eax\n" % thread
        cppString += "    shr $12, %eax\n"
        cppString += "    and $0xFF, %eax\n"
        cppString += "    or $0x00004600, %eax\n"
        #cppString += "    mov $0x00004600 + (thread%d_routine / 0x1000), %%eax\n" % thread
        cppString += "    mov PIC_ICR_LOW_ADDRESS, %esi\n"
        cppString += "    mov %eax, (%esi)\n"
        cppString += "    PIT_SLEEP_TICKS $10\n"
        cppString += "    mov %eax, (%esi)\n"
        threadIdx += 1
    cppString += "    movl $0x22222222, TEMP_DATA0\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $4\n"
    if thread == (numCores-1):
        cppString += "    # Primary core runs the last thread if no secondary core is available\n"
        cppString += "    jmp thread%d_routine\n" % thread
        # "wait_for_test_threads" is the return point
        cppString += "    .globl wait_for_test_threads\n"
    cppString += "wait_for_test_threads:\n"
    cppString += "    cmpb $NUM_THREADS, thread_join_lock\n"
    cppString += "    jne wait_for_test_threads\n"
    cppString += "    movl $0x33333333, TEMP_DATA0\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $4\n"
    #cppString += "#\n"
    #cppString += "# Construct linear hash table\n"
    #cppString += "#\n"
    #cppString += "# Register usage\n"
    #cppString += "#  -0x4(%ebp): current execution count\n"
    #cppString += "#  -0x8(%ebp): hash table size (number of hash key)\n"
    #cppString += "#  -0xC(%ebp): current index in hash table \n"
    #cppString += "#  eax: current signature from bss section\n"
    #cppString += "#  ebx:\n"
    #cppString += "#  ch: current thread count\n"
    #cppString += "#  cl: current sub-signature count\n"
    #cppString += "#  edx/rdx: temporary data\n"
    #cppString += "#  esi: address in bss section or in hash table\n"
    #cppString += "#  edi: second address\n"
    #cppString += "#  r10d: temporary\n"
    #cppString += "#  r11d: temporary\n"
    #cppString += "#\n"
    #cppString += "create_hash_table:\n"
    #cppString += "    push $0x0   /* -0x4(%ebp) both 32-bit and 64-bit */\n"
    #cppString += "    push $0x0   /* -0x8(%ebp) both 32-bit and 64-bit */\n"
    #cppString += "    push $0x0   /* -0xC(%ebp) both 32-bit and 64-bit */\n"
    #cppString += "execution_loop:\n"
    #cppString += "    movl $0x0, -0xC(%ebp)\n"
    #cppString += "hash_search_loop:\n"
    #cppString += "    movl -0xC(%ebp), %edx\n"
    #cppString += "    cmp -0x8(%ebp), %edx\n"
    #cppString += "    jge hash_entry_notfound  # jump if -0xC(%ebp) >= -0x8(%ebp)\n"
    #cppString += "    mov $0x0, %ch\n"
    #cppString += "1: # thread_loop\n"
    #cppString += "    mov $0x0, %cl\n"
    #cppString += "2: # signature-word loop\n"
    #cppString += "    ## Get value from bss section\n"
    #cppString += "    mov $TEST_BSS_SECTION, %esi\n"
    #cppString += "    mov -0x4(%ebp), %edx\n"
    #cppString += "    # %%esi <= %%esi + %%edx * %d\n" % signatureSize
    #for i in range(0, signatureSize, regBitWidth / 8):
    #    cppString += "    lea (%%esi,%%edx,%d), %%esi\n" % (regBitWidth / 8)
    #cppString += "    movzx %cl, %edx\n"
    #cppString += "    lea (%%esi,%%edx,%d), %%esi\n" % (regBitWidth / 8)
    #cppString += "    movzx %ch, %eax\n"
    #cppString += "    mov $TEST_BSS_SIZE_PER_THREAD, %edx\n"
    #cppString += "    mull %edx\n"
    #cppString += "    add %eax, %esi\n"
    #cppString += "    ## Get value from hash key table\n"
    #cppString += "    mov $TEST_HASH_KEY_TABLE, %edi\n"
    #cppString += "    mov -0xC(%ebp), %eax\n"
    #cppString += "    shll $%d, %%eax\n" % (int(math.ceil(math.log(signatureSize * len(threadList),2))))
    #cppString += "    add %eax, %edi\n"
    #cppString += "    movzx %cl, %edx\n"
    #cppString += "    lea (%%edi,%%edx,%d), %%edi\n" % (regBitWidth / 8)
    #cppString += "    movzx %ch, %edx\n"
    #cppString += "    # %%edi <= %%edi + %%edx * %d\n" % signatureSize
    #for i in range(0, signatureSize, regBitWidth / 8):
    #    cppString += "    lea (%%edi,%%edx,%d), %%edi\n" % (regBitWidth / 8)
    #cppString += "    # Compare\n"
    #cppString += "    mov (%edi), %rdx\n"
    #cppString += "    cmp %rdx, (%esi)\n"
    #cppString += "    jne move_next_hash\n"
    #cppString += "    inc %cl\n"
    #cppString += "    cmp $SIGNATURE_SIZE_IN_WORD, %cl  # subsignature count\n"
    #cppString += "    jl 2b\n"
    #cppString += "    inc %ch\n"
    #cppString += "    cmp $NUM_THREADS, %ch  # thread count\n"
    #cppString += "    jl 1b\n"
    #cppString += "    # Fall through\n"
    #cppString += "hash_entry_found:\n"
    #cppString += "    # Increase the hash value by 1\n"
    #cppString += "    mov $TEST_HASH_VALUE_TABLE, %esi\n"
    #cppString += "    movl -0xC(%ebp), %edx\n"
    #cppString += "    lea (%esi,%edx,4), %esi\n"
    #cppString += "    incl (%esi)\n"
    #cppString += "    jmp move_next_execution\n"
    #cppString += "move_next_hash:\n"
    #cppString += "    incl -0xC(%ebp)\n"
    #cppString += "    jmp hash_search_loop\n"
    #cppString += "hash_entry_notfound:\n"
    #cppString += "    ## Create a hash value entry (with value 1)\n"
    #cppString += "    mov $TEST_HASH_VALUE_TABLE, %esi\n"
    #cppString += "    movl -0x8(%ebp), %edx\n"
    #cppString += "    lea (%esi,%edx,4), %esi\n"
    #cppString += "    movl $1, (%esi)\n"
    #cppString += "    ## Create an hash key entry\n"
    #cppString += "    mov $0x0, %ch\n"
    #cppString += "1: # thread loop\n"
    #cppString += "    mov $0x0, %cl\n"
    #cppString += "2: # signature-word loop\n"
    #cppString += "    mov $TEST_BSS_SECTION, %edi\n"
    #cppString += "    mov -0x4(%ebp), %edx\n"
    ...
    #cppString += "    mull %edx\n"
    #cppString += "    add %eax, %edi\n"
    #cppString += "    mov $TEST_HASH_KEY_TABLE, %esi\n"
    #cppString += "    mov -0x8(%ebp), %eax\n"
    #cppString += "    shll $%d, %%eax\n" % (int(math.ceil(math.log(signatureSize * len(threadList),2))))
    #cppString += "    add %eax, %esi\n"
    #cppString += "    movzx %cl, %edx\n"
    #cppString += "    lea (%%esi,%%edx,%d), %%esi\n" % (regBitWidth / 8)
    ...
    #for i in range(0, signatureSize, regBitWidth / 8):
    #    cppString += "    lea (%%esi,%%edx,%d), %%esi\n" % (regBitWidth / 8)
    #cppString += "    # Copy value from (%edi) to (%esi)\n"
    #cppString += "    mov (%edi), %rdx\n"
    #cppString += "    mov %rdx, (%esi)\n"
    #cppString += "    inc %cl\n"
    #cppString += "    cmp $SIGNATURE_SIZE_IN_WORD, %cl  # subsignature count\n"
    #cppString += "    jl 2b\n"
    #cppString += "    inc %ch\n"
    #cppString += "    cmp $NUM_THREADS, %ch  # thread count\n"
    #cppString += "    jl 1b\n"
    #cppString += "    incl -0x8(%ebp)\n"
    #cppString += "    # Fall through\n"
    #cppString += "    # END of hash_entry_not_found\n"
    #cppString += "move_next_execution:\n"
    #cppString += "    incl -0x4(%ebp)\n"
    #cppString += "    cmpl $EXECUTION_COUNT, -0x4(%ebp)\n"
    #cppString += "    jl execution_loop\n"
    #cppString += "print_result:\n"
    #cppString += "    mov -0x4(%ebp), %edx\n"
    #cppString += "    mov %edx, TEMP_DATA0\n"
    #cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $4\n"
    #cppString += "    mov -0x8(%ebp), %edx\n"
    #cppString += "    mov %edx, TEMP_DATA0\n"
    #cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $4\n"
    ##for i in range(4):
    ##    cppString += "    VGA_PRINT_BYTES_64BIT $(TEST_HASH_KEY_TABLE + 32 * %d), $32\n" % i
    #cppString += "    VGA_PRINT_BYTES_64BIT $TEST_HASH_VALUE_TABLE, $32\n"
    #cppString += "    addl $0x18, %esp  /* 64-bit */\n"
    #cppString += "    # End of create_hash_table\n"
    cppString += "\n"
    for threadIdx in range(len(threadList)):
        cppString += "    VGA_PRINT_BYTES_64BIT $(TEST_BSS_SECTION + TEST_BSS_SIZE_PER_THREAD * %d), $32\n" % threadIdx
    cppString += "\n"
    cppString += "    mov $TEST_HASH_STACK_BASE, %esp\n"
    cppString += "    call classify_result_binary\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $(result_addr+16), $4\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $(result_addr+24), $4\n"
    cppString += "\n"
    cppString += "    # Infinite loop\n"
    cppString += "    jmp .\n"
    cppString += "PIT_SLEEP_TICKS_GLOBALS\n"
    cppString += "IDT_IA32E_48_ENTRIES\n"
    cppString += "interrupt_handler:\n"
    cppString += "    cmp IRQ_PIT, 8(%rsp)\n"
    cppString += "    jne not_pit\n"
    cppString += "    PIT_SLEEP_TICKS_HANDLER_UPDATE\n"
    cppString += "    ret\n"
    cppString += "not_pit:\n"
    cppString += "    mov 8(%rsp), %rax\n"
    cppString += "    mov %rax, TEMP_DATA0\n"
    cppString += "    VGA_PRINT_BYTES_64BIT $TEMP_DATA0, $8\n"
    cppString += "    ret\n"
